...
async function check() {
    ...
    if (await testServer(primaryServerIP)) {
        ...
        if (primaryFaultsDetected > 0)
            primaryFaultsDetected--;
    } else {
        // It is unavailable increase fault count if necessary
        if (primaryFaultsDetected <= nrOfFaults)
            primaryFaultsDetected++;
    }
    ...
    if (await testServer(secondaryServerIP)) {
        // It is available decrease fault count if necessary
        if (secondaryFaultsDetected > 0)
            secondaryFaultsDetected--;
    } else {
        // It is unavailable increase fault count if necessary
        if (secondaryFaultsDetected <= nrOfFaults)
            secondaryFaultsDetected++;
    }
...
async function shouldBe(ip) {
    ...
    if (IPv4.length === 0) {
        ...
        for (let record of IPv4) {
            if (record.data !== ip) {
                console.log("Update A-record from address " + record.data + " to address " + ip);
                record.data = ip;
