...
class Forwards {
    ...
    public async getForward(domainId: number, host: string): Promise<IForward> {
        const res = await this.api.apiCall("GET", `/domains/${domainId}/forwards/${host}`);
        return res.data;
    }
     ...
     */
    public async createForward(domainId: number, forward: IForward): Promise<string> {
        const res = await this.api.apiCall("POST", `/domains/${domainId}/forwards`, forward);
     ...
     */
    public async modifyForward(domainId: number, host: string, forward: IForward): Promise<void> {
        await this.api.apiCall("PUT", `/domains/${domainId}/forwards/${host}`, forward);
    }
